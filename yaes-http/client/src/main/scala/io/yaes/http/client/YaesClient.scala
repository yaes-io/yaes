...
/** Effect-based HTTP client built on Java's [[java.net.http.HttpClient]].
  ...
  * are raised as [[ConnectionError]] via the [[io.yaes.Raise]] effect; HTTP-level errors
  ...
  * client's lifecycle through the [[io.yaes.Resource]] effect.
  *
  * Example:
  * {{{
  * Resource.run {
...
object YaesClient:
  /** Creates a new [[YaesClient]] managed by the [[io.yaes.Resource]] effect.
    *
